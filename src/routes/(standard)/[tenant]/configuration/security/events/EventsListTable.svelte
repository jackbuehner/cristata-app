<script lang="ts">
  ...
  import type { EventsListQuery } from '$graphql/graphql';
  import type { StoreReturnType } from '$graphql/query';
  ...
  export let data: { _id: string; label: string; at: Date }[];
  ...
  <div role="table" class:compact={$table.options.meta?.compactMode}>
    ...
    <div role="rowgroup" class="tbody">
      {#each $table.getRowModel().rows as row, i}
        {@const href = `/${$page.params.tenant}/configuration/security/events/${row.getValue('_id')}`}
        {#key href}
          <a
            role="row"
            {href}
          ...
          >
            {#each row.getVisibleCells() as cell}
              {@const columnSize = cell.column.getSize() === 9999 ? '100%' : cell.column.getSize() + 'px'}
