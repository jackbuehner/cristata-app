...
function SelectionOverlay(props: ISelectionOverlay) {
  ...
  // whether the element is displayed using display
...
const Scrim = styled.div<{ isVisible: boolean }>`
...
const Content = styled.div<{ isVisible: boolean; isCompact?: boolean; theme: themeType }>`
  ...
  width: calc(100% - ${({ isCompact }) => (isCompact ? 0.2 : 0.4)}in);
  padding: ${({ isCompact }) => (isCompact ? '0.05in 0.1in' : '0.2in')};
  height: fit-content;
  box-sizing: border-box;
  box-shadow: 0.6px 0.6px 2.2px rgba(0, 0, 0, 0.02), 1.3px 1.3px 5.3px rgba(0, 0, 0, 0.028),
