<script lang="ts">
  ...
  async function addUser(selectedIds: string[]): Promise<boolean> {
    const HIDE_ITEMS = gql`
      mutation {
        ${selectedIds.map((item, index) => {
          return `hideItem${index}: ${uncapitalize(schemaName)}Hide(${byOne}: "${item}") {
            hidden
          }`;
        })}
      }
...
<ContentDialog
  title="Delete selected item{selectedIds.length === 1 ? '' : 's'}?"
  bind:open
  size="standard"
  style="
...
>
  ...
  <InfoBar severity="caution" title="This action may be permanent." />
  ...
  <svelte:fragment slot="footer">
    ...
    >
      ...
      {:else}
        Yes, delete item{selectedIds.length === 1 ? '' : 's'}
      {/if}
    </Button>
    <Button
      on:click={async () => {
