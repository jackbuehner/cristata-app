<script lang="ts">
  import { cristataCodeDarkTheme } from '$react/configuration/cristataCodeDarkTheme';
  ...
  export let wsProvider: YStore['wsProvider'] | undefined = undefined;
  ...
  $: ySharedType = $ydoc?.getText(ydocKey);

  // bind yjs to the monaco editor
  let binding: MonacoBinding | null = null;
  $: if (!binding && $ydoc && ySharedType && editor && model && $wsProvider) {
    binding = new MonacoBinding(ySharedType, model, new Set([editor]), $wsProvider?.awareness);
  }
  ...
  $: previewText = unified().use(remarkParse).use(remarkRehype).use(rehypeStringify).process(value);
