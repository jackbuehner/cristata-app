<script lang="ts">
  ...
  function handleUpdateOption(newOption: Option | null) {
    ...
    const isDifferent = JSON.stringify(oldSelectedOption) !== JSON.stringify(newOption || selectedOption);
...
{:else if options}
  ...
  {#key filteredItems}
    <ComboBox
      class="combobox-cristata"
      items={filteredItems}
      placeholder={showCurrentSelectionOnDropdown && selectedOption ? selectedOption.label : 'Select...'}
      {disabled}
      on:select={(evt) => {
        if (!evt.detail) return;
