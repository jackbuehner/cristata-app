<script lang="ts">
  ...
  let windowHeight = 0;
  afterNavigate(() => {
    collapsedFlyoutOpen = Object.fromEntries(Object.entries(collapsedFlyoutOpen).map(([key]) => [key, false]));
  });
</script>
  ...
  {#each (tree || []).filter((tr) => tr.name !== 'footer') as { name, path, type, pages, icon, onClick, selected }}
    ...
    {:else if type === 'category' || type === 'expander'}
      {#if __depth > 0 || type === 'expander'}
        ...
        {@const top = (flyoutRect?.top || 0) + flyoutOffScreenPixels === 0 ? 33 + 8 : flyoutRect?.top || 0}
        <div
          class="subtree"
          class:expanded
          style="
