<script lang="ts">
  ...
  function saveCommentReply() {
    ...
    const saved = editor
      ...
      .setComment({
        ...comment.attrs,
        replies: [...comment.attrs.replies.filter((reply) => reply.uuid !== uuid), { ...thisReply, message }],
      })
      .focus()
      .scrollIntoView()
      .run();
...
{#if thisReply}
  <div class="reply-container">
    ...
    {:else}
      ...
      <div class="timestamp">{DateTime.fromISO(thisReply.timestamp).toFormat(`LLL. dd, yyyy, t`)}</div>
    {/if}
  </div>
{/if}
