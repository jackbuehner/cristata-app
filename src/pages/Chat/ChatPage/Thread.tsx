...
import { IGetDiscussionComments, IGetDiscussionSingle } from '../../../interfaces/github/discussions';
...
const ThreadComponent = styled.div<{ theme: themeType }>`
  width: 360px;
  flex-shrink: 0;
  border-left: ${({ theme }) => `1px solid ${theme.color.neutral[theme.mode][300]}`};
`;
...
function Thread(props: IThread) {
  ...
  const [{ data: parentData, loading: parentLoading }, parentRefetch] = useAxios<IGetDiscussionSingle>(
    ...
    <ThreadComponent theme={theme}>
          ...
          <>
            <IconButton
              ...
              onClick={() => {
                ...
                history.push(location.pathname.replace(`/${props.discussionNumber}`, ''));
              }}
            />
            <IconButton
      ...
      {parentData ? (
        <Message
          ...
          time={
            `${
              DateTime.fromISO(parentData.organization.team.discussion.publishedAt)
                .toRelativeCalendar()!
                .charAt(0)
                .toUpperCase() +
              DateTime.fromISO(parentData.organization.team.discussion.publishedAt)
                .toRelativeCalendar()
                ?.slice(1)
            } at ${DateTime.fromISO(parentData.organization.team.discussion.publishedAt).toLocaleString(
          ...
          isSubscribed={parentData.organization.team.discussion.viewerSubscription === 'SUBSCRIBED'}
          canPin={parentData.organization.team.discussion.viewerCanPin}
          canSubscribe={parentData.organization.team.discussion.viewerCanSubscribe}
          canReact={parentData.organization.team.discussion.viewerCanReact}
          canReply={false}
          isThreadView={true}
        />
      ) : null}
      {data?.organization.team.discussion.comments.edges.map(({ node: discussion }, index: number) => {
        return (
          <Message
            key={index}
            ...
            reactionGroups={discussion.reactions.totalCount > 0 ? discussion.reactionGroups : undefined}
            replyCount={0}
            isPinned={false}
            isSubscribed={false}
            canPin={false}
